...
import { Calculator, CheckCircle2, Stamp, Printer, User } from "lucide-react";
...
export default function QuoteApprovals({ order, setOrder, admin }) {
  ...
  // --- Robust gating: require non-empty trimmed client name ---
  const clientName = useMemo(
    () => String(order?.client?.clientName ?? ""),
    [order?.client?.clientName]
  );
  const clientNameReady = clientName.trim().length > 0;
  const canQuote = clientNameReady && !busy;

  // Keep a small log breadcrumb so Debug Log can help if this ever feels stuck
  React.useEffect(() => {
      ...
      setOrder((o) => ({
        ...
        audit: [
          ...(o.audit || []),
          {
            ...
            detail: `Total ${quote.currency} ${quote.total}`,
          },
        ],
      }));
    } finally {
  ...
  function clientAccept() {
    ...
    setOrder((o) => ({
      ...
      audit: [
        ...(o.audit || []),
        { at: nowISO(), by: who, action: "CLIENT_ACCEPTED", detail: "" },
      ],
    }));
  }
  ...
  function ceoApprove(name) {
    ...
    setOrder((o) => ({
      ...
      audit: [
        ...(o.audit || []),
        { at: nowISO(), by: n, action: "CEO_APPROVED", detail: "" },
      ],
    }));
  }
  ...
  return (
    <Section title="Quote & Approvals" icon={<Calculator className="h-6 w-6 text-gray-800" />}>
      {/* Live “we detected client name” chip */}
      ...
      {/* Friendly reason if disabled */}
      {!clientNameReady && (
        <div className="md:col-span-2 mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-xs text-amber-800">
          To generate a quote, please enter the <strong>Client Name</strong> in the
          <strong> Client Intake</strong> section.
        </div>
      )}
      ...
      <div className="md:col-span-2 rounded-xl border border-gray-200 p-3">
        ...
        {q ? (
          <div className="text-sm text-gray-800">
            ...
            <div>
              Subtotal: {fmt(q.subtotal, q.currency)} | Shipping: {fmt(q.shipping, q.currency)} | Tax:{" "}
              {fmt(q.tax, q.currency)}
            </div>
